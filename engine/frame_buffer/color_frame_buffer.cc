...
std::vector<GLubyte> ColorFrameBuffer::GetColorTextureData(int i) {
  ...
  glGetTexLevelParameteriv_(target, 0, GL_TEXTURE_WIDTH, &width);
  glGetTexLevelParameteriv_(target, 0, GL_TEXTURE_HEIGHT, &height);
  glGetTexLevelParameteriv_(target, 0, GL_TEXTURE_INTERNAL_FORMAT, &internal_format);
  ...
  glGetTexImage_(target, 0, format, type, res.data());
