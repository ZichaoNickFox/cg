...
std::shared_ptr<engine::Texture> CreateTexture2D(int width, int height, int format) {
  ...
  return std::make_shared<engine::Texture>(texture, Texture::Texture2D);
}

std::shared_ptr<Texture> LoadTexture2D(const std::string& path_with_ext, bool useMipmap) {
...
std::shared_ptr<Texture> CreateFromData( GLubyte* data, int width, int height, bool useMipmap) {
  GLuint ret;
  glGenTextures(1, &ret);
  glBindTexture(GL_TEXTURE_2D, ret);
