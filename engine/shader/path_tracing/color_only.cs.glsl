...
void main() {
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    if (hit_result.hitted && hit_result.dist >= camera_geometry.near
        && hit_result.dist <= camera_geometry.far && t_min > hit_result.dist) {
...
//  imageStore(texture_output, ivec2(gl_GlobalInvocationID.xy), vec4(dir, 1.0));
}