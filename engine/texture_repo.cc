...
std::shared_ptr<Texture> TextureRepo::GetOrLoadTexture(const std::string& name) {
  ...
  if (!state->loaded) {
    if (state->texture_type == Texture::Texture2D) {
      ...
      state->texture = LoadTexture2D(state->paths[0], true);
    } else if (state->texture_type == Texture::CubeMap) {
      state->texture = LoadCubeMap(state->paths);
    } else {
...
std::shared_ptr<Texture> TextureRepo::LoadTexture2D(const std::string& path_with_ext, bool useMipmap){
...
// failed return 0
bool TextureRepo::SaveTexture2D(const std::string& path_with_ext, int width,
...
bool TextureRepo::SaveTexture2D(const std::string& path_with_ext, GLuint tex) {
...
std::shared_ptr<Texture> TextureRepo::LoadCubeMap(const std::vector<std::string>& path) {
  GLuint textureId;
  glGenTextures(1, &textureId);
  
  std::shared_ptr<Texture> ret = std::make_shared<Texture>(textureId, Texture::CubeMap);
