...
bool Object::Intersect(Context* context, const glm::vec3& origin_ws, const glm::vec3& dir_ws,
  ...
  glm::vec3 dir_ls = inverse_model * glm::vec4(dir_ws, 1.0);
  glm::vec3 vertex0_ls, vertex1_ls, vertex2_ls, normal_ls, position_ls;
  float distance_ls;
  if (!CGCHECK_NOTNULL(mesh(context))->Intersect(origin_ls, dir_ls, &position_ls, &normal_ls, &distance_ls,
                                                 &vertex0_ls, &vertex1_ls, &vertex2_ls)) {
    return false;
  }
  result->pos_ws = model_matrix * glm::vec4(position_ls, 1.0);
  result->normal_ws = model_matrix * glm::vec4(normal_ls, 1.0);
