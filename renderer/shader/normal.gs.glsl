...
void LineWidth(vec3 pos0_ws, vec3 pos1_ws, vec3 color0, vec3 color1, vec3 view_pos_ws, float width) {
  vec3 extension_dir = normalize(cross(view_pos_ws - pos0_ws, pos1_ws - pos0_ws));
...
void GenVertexNormal(int index) {
  ...
  LineWidth(pos0_ws, pos1_ws, color, color, view_pos_ws, line_width);
}
...
void GenTBN(vec3 TBN_coord_world_pos, vec3 TBN_coord_world_axis, vec4 color) {
  ...
  LineWidth(pos0_ws, pos1_ws, vec3(color), vec3(color), view_pos_ws, line_width);
}
